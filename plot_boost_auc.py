...
for kidx in range(len(ks)):
    ...
    for midx in range(6):
        im[kidx].plot(range(X_max),train_roc[kidx,midx,:X_max],colors[midx]+'-.',range(X_max),test_roc[kidx,midx,:X_max],colors[midx]+'-o',label='m='+str(midx))

    im[kidx].set_xlabel('Boosting Round',fontsize=12)
    im[kidx].set_ylabel('Mean AUC',fontsize = 12)
    im[kidx].set_title('k = '+str(ks[kidx]),fontsize=14)
    leg = im[kidx].legend(loc=4,labelspacing=0.1)
    for l in leg.get_texts():
        l.set_fontsize('10')

    outfile = '/proj/ar2384/picorna/Adaboost/boosterror_10mer_%d.pdf' % ks[kidx]
