...
for i,train_indices in enumerate(split_indices):
    test_indices = set(range(Y.shape[1])).difference(train_indices)
    # train
    train_features = [labelled_featuresets[i] for i in train_indices]
    model = nb.NaiveBayesClassifier.train(train_featuresets)
    # test
    test_features = [labelled_featuresets[i] for i in test_indices]
    label = [model.classify(featureset) for featureset in test_features]
    # collect
